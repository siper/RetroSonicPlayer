...
public class MediaNotificationProcessor {
    ...
    /**
     * The lightness difference that has to be added to the primary text color to obtain the
     * secondary text color when the background is dark. A bit less then the above value, since it
    ...
    private void ensureColors(int backgroundColor, int mForegroundColor) {
        {
            ...
            double contrast =
                    NotificationColorUtil.calculateContrast(mForegroundColor, backgroundColor);
            ...
            if (contrast < 4.5f) {
                ...
                if (NotificationColorUtil.calculateContrast(secondaryTextColor, backgroundColor)
                        < 4.5f) {
