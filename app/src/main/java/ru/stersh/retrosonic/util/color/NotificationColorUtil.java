 ...
 *
 */
package ru.stersh.retrosonic.util.color;
...
public class NotificationColorUtil {
    ...
    //    /**
    //     * Inverts all the grayscale colors set by {@link android.text.style.TextAppearanceSpan}s
    // on
    //     * the text.
    //     *
    ...
    //
    //                    // Allocate a new array so we don't change the colors in the old color
    // state
    //                    // list.
    //                    if (!changed) {
     ...
     *     increase in the LAB color space. A negative value will darken the color and a positive
     *     will lighten it.
    ...
    //     * @param isDark whether or not the {@code notificationColor} will be placed on a
    // background
                ...
                Log.w(
                        ...
                        String.format(
                                "Ambient contrast of notification for %s is %s (over black)"
                                        + " by changing #%s to #%s",
                                context.getPackageName(),
                                NotificationColorUtil.contrastChange(
                                        resolvedColor, color, Color.BLACK),
