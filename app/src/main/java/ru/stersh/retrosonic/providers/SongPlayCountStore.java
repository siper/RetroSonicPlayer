 ...
 *
 */
package ru.stersh.retrosonic.providers;
...
public class SongPlayCountStore extends SQLiteOpenHelper {
    ...
    @NonNull private static final Interpolator sInterpolator = new AccelerateInterpolator(1.5f);
    ...
    @NonNull private static final String WHERE_ID_EQUALS = SongPlayCountColumns.ID + "=?";
    ...
    /**
     * For some performance gain, return a static value for the column index for a week WARNING:
     * This function assumes you have selected all columns for it to work
     *
     * @param week number
    ...
    private void updateExistingRow(
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            int lastUpdatedIndex =
                    cursor.getColumnIndex(SongPlayCountColumns.LAST_UPDATED_WEEK_INDEX);
            ...
            if (Math.abs(weekDiff) >= NUM_WEEKS) {
                ...
                if (score < .01f) {
                    ...
                    values.put(
                            SongPlayCountColumns.LAST_UPDATED_WEEK_INDEX, mNumberOfWeeksSinceEpoch);
    ...
    /**
     * This updates all the results for the getTopPlayedResults so that we can get an accurate list
     * of the top played results
     */
