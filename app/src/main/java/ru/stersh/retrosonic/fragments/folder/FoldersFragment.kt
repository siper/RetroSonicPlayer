 ...
 *
 */
package ru.stersh.retrosonic.fragments.folder
...
import ru.stersh.retrosonic.adapter.SongFileAdapter
import ru.stersh.retrosonic.adapter.Storage
import ru.stersh.retrosonic.adapter.StorageAdapter
import ru.stersh.retrosonic.adapter.StorageClickListener
import ru.stersh.retrosonic.databinding.FragmentFolderBinding
...
import ru.stersh.retrosonic.interfaces.IMainActivityFragmentCallbacks
import ru.stersh.retrosonic.misc.UpdateToastMediaScannerCompletionListener
import ru.stersh.retrosonic.misc.WrappedAsyncTaskLoader
import ru.stersh.retrosonic.model.Song
import ru.stersh.retrosonic.util.FileUtil
import ru.stersh.retrosonic.util.PreferenceUtil.startDirectory
import ru.stersh.retrosonic.util.ThemedFastScroller.create
import ru.stersh.retrosonic.util.defaultNavOptions
import ru.stersh.retrosonic.util.getExternalStorageDirectory
import ru.stersh.retrosonic.util.getExternalStoragePublicDirectory
import ru.stersh.retrosonic.views.BreadCrumbLayout.Crumb
import ru.stersh.retrosonic.views.BreadCrumbLayout.SelectionCallback
import java.io.File
...
class FoldersFragment :
    AbsMainActivityFragment(R.layout.fragment_folder),
...
//        mainActivity.addMusicServiceEventListener(libraryViewModel)
    ...
    override fun onFileMenuClicked(file: File, view: View) {
        ...
        if (file.isDirectory) {
            ...
            popupMenu.setOnMenuItemClickListener { item: MenuItem ->
                when (val itemId = item.itemId) {
                    R.id.action_play_next, R.id.action_add_to_current_playing, R.id.action_add_to_playlist, R.id.action_go_to_album, R.id.action_go_to_artist, R.id.action_share, R.id.action_details, R.id.action_set_as_ringtone, R.id.action_delete_from_device -> {
                        lifecycleScope.launch(Dispatchers.IO) {
                            listSongs(
                                requireContext(),
                                listOf(file),
...
//                            openQueue(songs, startIndex, true)
                            ...
                            Snackbar.make(
                                ...
                                String.format(
                                    getString(R.string.not_listed_in_media_store),
                                    mFile.name,
    ...
    override fun onCreateMenu(menu: Menu, inflater: MenuInflater) {
        ...
        ToolbarContentTintHelper.handleOnCreateOptionsMenu(
            ...
            toolbar,
            menu,
            ATHToolbarActivity.getToolbarBackgroundColor(
                toolbar,
            ),
        )
    }
